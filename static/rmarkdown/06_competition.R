...
get_fec <- function(timesteps, fec_mu, vcov){
  ...
  vcov[1,2] <- vcov[2,1] <- fec_cor*fec_sigma[1]*fec_sigma[2]
  # Now we draw from the multivariate normal:
  fec <- rmvnorm(timesteps, mean=fec_mu, sigma=vcov) # draw fecundities from a distribution
...
for(i in 2:timesteps){
  Nmatrix[i,] <- DD_2spp_growth(N=Nmatrix[i-1,],g,s,fec[i,],alpha)
}
...
for(i in 1:invasion_reps){
  N_t1[i,] <- DD_2spp_growth(N=N_t0[i,],g,s,fec[i,],alpha_invade) 
